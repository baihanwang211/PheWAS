...
# load bmi GWAS
bmi_EAS <- data.frame(fread("./gwas/harmonised/bmi_EAS.gwaslab.tsv.gz"))

bmi_EAS$Phenotype <- "bmi"

bmi_EAS_clump <- ld_clump(
  dplyr::tibble(rsid=bmi_EAS$rsID, pval=bmi_EAS$P, id=bmi_EAS$Phenotype),
  plink_bin = "/home/baihanw/.conda/envs/plink/bin/plink",
  bfile = "/home/baihanw/reference/1kg/EAS")
...
# load smkinit GWAS
smkinit_EAS <- data.frame(fread("./gwas/harmonised/smkinit_EAS.gwaslab.tsv.gz"))
...
# load cigday GWAS
cigday_EAS <- data.frame(fread("./gwas/harmonised/cigday_EAS.gwaslab.tsv.gz"))
...
# load scz_EAS GWAS
scz_EAS <- data.frame(fread("./gwas/harmonised/scz_EAS.gwaslab.tsv.gz"))
...
# load bmi GWAS
bmi_EUR <- data.frame(fread("./gwas/harmonised/bmi_EUR.gwaslab.tsv.gz"))
...
bmi_EUR_clump <- ld_clump(
  ...
  plink_bin = "/home/baihanw/.conda/envs/plink/bin/plink",
  bfile = "/home/baihanw/reference/1kg/EUR")
...
# load smkinit GWAS
smkinit_EUR <- data.frame(fread("./gwas/harmonised/smkinit_EUR.gwaslab.tsv.gz"))
...
# load cigday GWAS
cigday_EUR <- data.frame(fread("./gwas/harmonised/cigday_EUR.gwaslab.tsv.gz"))
...
# load scz_EUR GWAS
scz_EUR <- data.frame(fread("./gwas/harmonised/scz_EUR.gwaslab.tsv.gz"))
